...
def add_source(con, username, title=None, evidence='', origin='unknown'):
    ...
    if not re.fullmatch(r'[A-Za-z0-9_]{5,}', username): return False
    if source_exists(con, username): return False
...
def cached_channel_candidates():
    if not SESSION_DB.exists(): return []
    ...
    for peer_id, username, name, _ in rows:
        # Telethon stores marked peer IDs: users are positive, chats/channels negative.
        # Only negative IDs are allowed into source discovery to avoid ever treating people as sources.
        if not isinstance(peer_id, int) or peer_id >= 0: continue
        text = f'{username or ""} {name or ""}'
        if useful(text): out.append((username, name or username))
    return out
...
async def cached_entity_discovery(con):
    ...
    if candidates: set_state('cache_cursor', (cursor + len(batch)) % len(candidates))
    added = 0
...
async def graph_discovery(con):
    ...
    added = 0; refs_seen = 0; sources_read = 0
    cached_names = {u.lower() for u, _ in cached_channel_candidates()}
    for row in batch:
        ...
        try:
            ...
            async for msg in client.iter_messages(entity, limit=GRAPH_MESSAGES_PER_SOURCE):
                if msg.message: evidence_parts.append(msg.message)
            evidence = '\n'.join(evidence_parts)
            for ref in extract_refs(evidence):
                refs_seen += 1
                # Never resolve a new username here. Only promote references that already exist
                # in the local discovery session cache and look job-related by cached metadata.
                if ref.lower() not in cached_names: continue
...
async def global_search_discovery(con, queries, cutoff):
    added = 0; examined = 0; unique = set()
    for query in queries:
        try:
            async for msg in client.iter_messages(None, search=query, limit=SEARCH_LIMIT_PER_QUERY):
                examined += 1
                if not msg.date or msg.date < cutoff: continue
                text = msg.message or ''
                if not useful(text): continue
                chat = await msg.get_chat()
                if not chat or not isinstance(chat, (Channel, Chat)): continue
...
async def main():
    ...
    if 'timezone' not in cols:
        con.execute('ALTER TABLE sources ADD COLUMN timezone TEXT'); con.commit()
